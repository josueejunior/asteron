...
static void compile_node(BytecodeProgram* program, ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_IDENTIFIER: {
            ...
            // CORREÇÃO CRÍTICA: Não gera RELEASE se a variável é usada como argumento
            // de função logo depois (detecta pelo contexto do nó pai)
            // Por enquanto, usa heurística conservadora: não libera se pode haver uso futuro
            if (node->as.identifier.is_last_use) {
                // CORREÇÃO: Verifica se o uso está em PRINT - se estiver, não libera
                // porque pode haver uso futuro em função (ex: tcp_close após print)
                // Isso é uma heurística temporária até melhorar a análise de liveness
                int should_release = 1;
                
                // Se o identificador está sendo usado em um PRINT, é mais seguro não liberar
                // porque pode haver uso futuro em chamadas de função
                // (Isso será melhorado com análise de fluxo de dados mais sofisticada)

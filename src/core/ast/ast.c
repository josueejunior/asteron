...
const char* annotation_type_to_string(AnnotationType type) {
    switch (type) {
        ...
        case ANNOTATION_PURE: return "pure";
        case ANNOTATION_MEMOIZE: return "memoize";
        case ANNOTATION_LAZY: return "lazy";
        case ANNOTATION_HOT: return "hot";
        case ANNOTATION_INLINE: return "inline";
        case ANNOTATION_NOOPT: return "noopt";
        default: return "unknown";
    }
}
...
void ast_destroy_node(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case AST_PHI:
            ...
            break;
        case AST_IMPORT:
            string_free(node->as.import_stmt.module_name);
            if (node->as.import_stmt.symbols != NULL) {
                for (size_t i = 0; i < node->as.import_stmt.symbol_count; i++) {
                    string_free(node->as.import_stmt.symbols[i]);
                    if (node->as.import_stmt.aliases && node->as.import_stmt.aliases[i]) {
                        string_free(node->as.import_stmt.aliases[i]);
                    }
                }
                free(node->as.import_stmt.symbols);
                if (node->as.import_stmt.aliases) {
                    free(node->as.import_stmt.aliases);
                }
            }
            break;
        default:
...
void ast_print_node(ASTNode* node, int indent) {
    ...
    switch (node->type) {
        ...
        case AST_IMPORT:
            print_indent(indent);
            printf("Import(%s", node->as.import_stmt.module_name);
            if (node->as.import_stmt.symbol_count > 0) {
                printf(" : ");
                for (size_t i = 0; i < node->as.import_stmt.symbol_count; i++) {
                    printf("%s", node->as.import_stmt.symbols[i]);
                    if (node->as.import_stmt.aliases && node->as.import_stmt.aliases[i]) {
                        printf(" as %s", node->as.import_stmt.aliases[i]);
                    }
                    if (i < node->as.import_stmt.symbol_count - 1) printf(", ");
                }
            }
            printf(")\n");
            break;
...
ASTNode* ast_import(char* module_name, char** symbols, size_t symbol_count, char** aliases) {
    ...
    node->as.import_stmt.module_name = string_copy(module_name, strlen(module_name));
    node->as.import_stmt.symbol_count = symbol_count;
    
    if (symbol_count > 0 && symbols != NULL) {
        node->as.import_stmt.symbols = (char**)malloc(sizeof(char*) * symbol_count);
        node->as.import_stmt.aliases = aliases ? (char**)malloc(sizeof(char*) * symbol_count) : NULL;
        
        for (size_t i = 0; i < symbol_count; i++) {
            node->as.import_stmt.symbols[i] = string_copy(symbols[i], strlen(symbols[i]));
            if (aliases && aliases[i]) {
                node->as.import_stmt.aliases[i] = string_copy(aliases[i], strlen(aliases[i]));
            } else if (node->as.import_stmt.aliases) {
                node->as.import_stmt.aliases[i] = NULL;
            }
        }
    } else {
        node->as.import_stmt.symbols = NULL;
        node->as.import_stmt.aliases = NULL;
    }

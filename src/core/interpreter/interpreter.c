...
/* Funções de conversão entre Value (interpreter) e AsteronValue (ABI) */
static AsteronValue value_to_asteron_value(Value v) {
    if (v.type == VAL_NUMBER) {
        return ASTERON_NUMBER(v.as.number);
    } else if (v.type == VAL_BOOL) {
        return ASTERON_BOOL(v.as.boolean);
        ...
        if (v.as.obj->type == OBJ_STRING) {
            ...
            // Verifica se chars é válido
            size_t str_len = 0;
            if (str->chars) {
                str_len = str->length;
            } else if (str->length > 0) {
                // Se length > 0 mas chars é NULL, algo está errado
                return ASTERON_NIL();
            }
            
            // Aloca AsteronString (seguindo o padrão da VM)
            // IMPORTANTE: Aloca espaço suficiente para header + length + null terminator
            ...
            if (str_len > 0 && str->chars) {
                memcpy(astr->chars, str->chars, str_len);
...
static Value asteron_value_to_value(AsteronValue av) {
    if (ASTERON_IS_NUMBER(av)) {
        return value_number(ASTERON_AS_NUMBER(av));
    } else if (ASTERON_IS_BOOL(av)) {
        return value_bool(ASTERON_AS_BOOL(av) ? 1 : 0);
    } else if (ASTERON_IS_NIL(av)) {
        return value_nil();
    } else if (ASTERON_IS_STRING(av)) {
        AsteronString* astr = ASTERON_AS_STRING(av);
        if (!astr) {
            return value_nil();
        }
        ...
        // value_string vai copiar novamente internamente
        ...
        Value result = value_string(str_copy);
        free(str_copy); // value_string já copiou internamente
        return result;
    }
    return value_nil();
}
...
Value environment_get(Environment* env, const char* name) {
    ...
    // Não imprime erro aqui - deixa que o chamador decida se é erro
    // (pode ser uma função nativa que será resolvida na chamada)
    return value_nil();
}
...
const char* value_to_chars(Value value) {
    if (value_is_string(value)) {
        ObjString* str = (ObjString*)value.as.obj;
        if (str && str->chars) {
            return str->chars;
        }
    }
...
// Controle de retorno de funções (para comunicação com VM)
static int g_has_returned = 0;
static Value g_return_value = {.type = VAL_NIL};

int interpreter_has_returned(void) {
    return g_has_returned;
}
...
Value interpreter_evaluate_expression(ASTNode* expr) {
    ...
    switch (expr->type) {
        ...
        case AST_IDENTIFIER: {
            ...
            // Se não encontrou função Asteron, verifica se é função nativa
            // (para suportar referências a funções nativas como valores first-class)
            AsteronNativeFn native_fn = vm_find_native_function(expr->as.identifier.name);
            if (native_fn != NULL) {
                // Funções nativas não podem ser valores first-class diretamente
                // Mas podemos retornar nil e deixar que a chamada de função trate
                // Por enquanto, apenas não dá erro
                return value_nil();
            }
        ...
        case AST_FUNCTION_CALL: {
            ...
            // Se não encontrou função Asteron, tenta buscar função nativa
            if (func == NULL) {
                AsteronNativeFn native_fn = vm_find_native_function(expr->as.function_call.name);
                if (native_fn) {
                    // Avalia argumentos
                    size_t arg_count = expr->as.function_call.argument_count;
                    AsteronValue* args = NULL;
                    
                    if (arg_count > 0) {
                        args = (AsteronValue*)malloc(sizeof(AsteronValue) * arg_count);
                        if (args == NULL) {
                            return value_nil();
                        }
                        
                        for (size_t i = 0; i < arg_count; i++) {
                            Value v = interpreter_evaluate_expression(expr->as.function_call.arguments[i]);
                            args[i] = value_to_asteron_value(v);
                            value_destroy(v);
                        }
                    }
                    
                    // Chama função nativa
                    AsteronValue result = native_fn((int)arg_count, args);
                    
                    // NOTA: NÃO liberar strings de argumentos aqui
                    // Isso causa corrupção de heap (double-free)
                    // O vazamento de memória é temporário e será corrigido com GC
                    if (args) {
                        free(args);  // Apenas libera o array, não os conteúdos
                    }
                    
                    // Converte resultado
                    Value result_val = asteron_value_to_value(result);
                    
                    // NOTA: Não liberamos o resultado aqui porque:
                    // - O interpretador vai gerenciar via value_destroy
                    // - Se a função nativa retornou uma string com flag OWNED,
                    //   ela será liberada quando o Value for destruído
                    // - Mas precisamos garantir que a string foi copiada corretamente
...
void interpreter_execute_statement(ASTNode* stmt) {
    ...
    switch (stmt->type) {
        ...
        case AST_RETURN: {
            // Seta flags globais para comunicação com VM
            if (stmt->as.return_stmt.value) {
                g_return_value = interpreter_evaluate_expression(stmt->as.return_stmt.value);
                // Retém referência do valor de retorno
                if (g_return_value.type == VAL_OBJ) {
                    value_retain(g_return_value);
                    if (g_return_value.as.obj && g_return_value.as.obj->type == OBJ_STRING) {
                        ObjString* str = (ObjString*)g_return_value.as.obj;
                        fprintf(stderr, "[INTERPRETER] RETURN: String retida '%s' (len=%zu)\n", 
                                str->chars ? str->chars : "(null)", str->length);
                    }
                }
            } else {
                g_return_value = value_nil();
        ...
        case AST_IF_STATEMENT: {
            // Se já houve retorno, não executa
            if (g_has_returned) break;
        ...
        case AST_WHILE_STATEMENT: {
            for (;;) {
                // Verifica se houve retorno dentro do loop
                if (g_has_returned) break;
                ...
                // Verifica novamente após executar o corpo
                if (g_has_returned) break;
            }
            break;
        }
        ...
        case AST_BLOCK: {
            for (size_t i = 0; i < stmt->as.block.count; i++) {
                if (g_has_returned) break;  // Para se houve retorno

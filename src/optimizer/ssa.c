...
static void mark_last_use_backward(ASTNode* node, const char* name, int version, int* found, int in_loop) {
    ...
    if (node->type == AST_BLOCK) {
        for (int i = (int)node->as.block.count - 1; i >= 0; i--) {
            mark_last_use_backward(node->as.block.statements[i], name, version, found, in_loop);
            if (*found) return;
        }
    }
    ...
    if (node->type == AST_IDENTIFIER && 
        ...
        // Só marca como último uso se NÃO estivermos dentro de um loop que depende desta variável
        if (!in_loop) {
    ...
    switch (node->type) {
        case AST_VARIABLE_DECLARATION:
            mark_last_use_backward(node->as.variable_decl.value, name, version, found, in_loop);
            break;
        case AST_ASSIGNMENT:
            mark_last_use_backward(node->as.assignment.value, name, version, found, in_loop);
            break;
        case AST_BINARY_EXPRESSION:
            mark_last_use_backward(node->as.binary_expr.right, name, version, found, in_loop);
            mark_last_use_backward(node->as.binary_expr.left, name, version, found, in_loop);
            break;
        case AST_PRINT:
            mark_last_use_backward(node->as.print_stmt.expression, name, version, found, in_loop);
            break;
        case AST_IF_STATEMENT:
            mark_last_use_backward(node->as.if_stmt.else_branch, name, version, found, in_loop);
            mark_last_use_backward(node->as.if_stmt.then_branch, name, version, found, in_loop);
            mark_last_use_backward(node->as.if_stmt.condition, name, version, found, in_loop);
            break;
        case AST_WHILE_STATEMENT:
            mark_last_use_backward(node->as.while_stmt.body, name, version, found, in_loop);
            mark_last_use_backward(node->as.while_stmt.condition, name, version, found, in_loop);
            break;
        default: break;
    }
}
...
void ssa_analyze_liveness(ASTNode* node) {
    ...
    for (int i = 0; i < ssa_var_count; i++) {
        ...
        for (int v = 0; v < ssa_table[i].total_versions; v++) {
            int found = 0;
            mark_last_use_backward(node, ssa_table[i].name, v, &found, 0);
        }
    }
